...
const News = () => {
  ...
  return (
    <div className="bg-[#25282A] py-10 px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {newsItems.map((item, index) => (
            <div key={index} className="rounded-3xl overflow-hidden shadow-lg">
              <img src={item.image} alt={item.headline} className="w-full h-64 object-cover" />
              <div className="p-4">
                <p className="text-gray-400 text-sm">{item.date}</p>
                <h3 className="text-white text-lg font-semibold mb-2">{item.headline}</h3>
              </div>
            </div>
          ))}
        </div>
